### study-e-book
最近整理了一下以前百度云、自己电脑收藏的一些电子书，分享出来，有需要的自取。
自取的同时请大家[star](https://github.com/Han-Ya-Jun/study-e-book)下哈，整理不易。
</br> </br>注：*所有书籍类资源都逃不过版权问题，这个层面上还是希望有能力的人支持正版
书籍有版权，知识却没有，任何学习的行为都值得被鼓励*
### 提取方法
- 链接：https://pan.baidu.com/s/143-g1YVM4AD96KBUsUv87A 
- 提取码：wavk 

![](http://cdn.hanyajun.com/20190602_025454_baiduyun.jpg)
### 电子书目录
```
study-e-book
├── devops
│   └── Python自动化运维：技术与最佳实践.文字版.pdf
├── README.md
├── tree.md
├── 中间件
│   ├── Kafka权威指南.pdf
│   ├── RabbitMQ实战指南.pdf
│   ├── RocketMQ实战与原理解析.pdf
│   └── 各种消息中间件对比.pdf
├── 云原生
│   ├── (纪念版) Kubernetes权威指南  从Docker到Kubernetes实践全接触.pdf
│   ├── Docker—从入门到实践.pdf
│   ├── Docker从入门到实践.pdf
│   ├── Docker企业级架构篇.pdf
│   ├── Docker入门实战手册.pdf
│   ├── Spring Cloud与Docker微服务架构实战.pdf
│   ├── Spring+Cloud与Docker微服务架构实战 周立.pdf
│   ├── 【Docker实战】- Registry &Portus搭建详解.pdf
│   └── 第一本Docker书 高清完整版.pdf
├── 互联网
│   ├── 浪潮之巅.pdf
│   └── 观止__微软创建NT和未来的夺命狂奔.pdf
├── 其他
│   ├── 2017alitech_01.pdf
│   ├── 2017alitech_02.pdf
│   ├── [计算机程序的构造和解释].(中文版）.清晰版.pdf
│   └── 《精通正则表达式》第3版.pdf
├── 分布式
│   ├── [分布式系统概念与设计].(Distributed.Systems.C.pdf
│   ├── 分布式Java应用：基础与实践.pdf
│   ├── 分布式网络爬虫URL去重策略的改进.pdf
│   ├── 大型分布式网站架构 设计与实践.pdf
│   └── 大规模分布式系统架构与设计实战.pdf
├── 前端
│   ├── Bootstrap用户手册(清晰pdf).pdf
│   ├── Building APIs with Node.js.pdf
│   ├── CSS3属性速查表.pdf
│   ├── CSS世界-张鑫旭.pdf
│   ├── css揭秘.pdf
│   ├── CSS权威指南（第三版）(jb51.net).pdf
│   ├── CSS设计指南（第3版）.pdf
│   ├── ES6标准入门（第二版）.pdf
│   ├── flex布局.png
│   ├── HTML5与CSS3基础教程（第8版）中文高清版.pdf
│   ├── HTML5与CSS3权威指南第二版.pdf
│   ├── HTML5实例与案例.pdf
│   ├── HTML5实战.pdf
│   ├── HTML5标签列表.pdf
│   ├── HTML5移动Web开发指南.pdf
│   ├── JavaScript+DOM编程艺术.pdf
│   ├── javascript-promise-book.pdf
│   ├── javascript函数式编程.pdf
│   ├── JavaScript启示录.pdf
│   ├── JavaScript基础教程（第8版).pdf
│   ├── JavaScript权威指南(第5版)中文版(上).pdf
│   ├── JavaScript权威指南(第5版)中文版(下).pdf
│   ├── JavaScript权威指南-第6版.pdf
│   ├── JavaScript王者归来.pdf
│   ├── JavaScript设计模式.pdf
│   ├── JavaScript语言精粹.pdf
│   ├── JavaScript语言精粹[修订版].pdf
│   ├── JavaScript面向对象编程指南.pdf
│   ├── JavaScript高级程序设.pdf
│   ├── JavaScript高级程序设计（第3版）.pdf
│   ├── Node.js实战(第2季).pdf
│   ├── Node.js实战.pdf
│   ├── Node.js开发指南_中文正版.pdf
│   ├── Node.js高级编程  PDF电子书下载 带书签目录 完整版.pdf
│   ├── Node与Express开发.pdf
│   ├── Node即学即用(linuxidc.com).pdf
│   ├── promise迷你书.pdf
│   ├── React Native入门与实战[人民邮电出版社_P375_2016.01].pdf
│   ├── React 中文版 - v1.1.pdf
│   ├── React.js 2016 最佳实践.pdf
│   ├── ReactNative用javascript开发移动应用.pdf
│   ├── REACT：引领未来的用户界面开发框架.pdf
│   ├── RESTful Web API Design with Node.JS - Second Edition.pdf
│   ├── WebGL编程指南.pdf
│   ├── Web性能权威指南.pdf
│   ├── 《ajax从入门到精通》.(陈华).[PDF]@ckook.pdf
│   ├── 《Vue.js权威指南》.pdf
│   ├── 《天猫 React Native 实践与探索》 - 朱柯军 - 2015-05-30@杭州segmentfault.pdf
│   ├── 【JavaScript高级程序设计（第3版）】中文 高清 .pdf
│   ├── 【分享】koa-not-only-generator.pdf
│   ├── 一个月悟透JavaScript.pdf
│   ├── 你不知道的 JavaScript（下卷）.pdf
│   ├── 你不知道的JavaScript（上卷）.pdf
│   ├── 你不知道的JavaScript（中卷）.pdf
│   ├── 深入浅出Node.js.pdf
│   ├── 精通JavaScript.pdf
│   ├── 编写可维护的JavaScript.pdf
│   └── 高性能JavaScript.pdf
├── 大佬分享
│   ├── 2019GopherChina讲师PPT
│   │   ├── 2019-04-27
│   │   │   ├── 1.1 大型微服务框架设计实践 - 杜欢.pdf
│   │   │   ├── 1.2 用Go打造Grab的路径规划和ETA引擎.pdf
│   │   │   ├── 1.3 Go practices in TiDB - 姚维.pdf
│   │   │   ├── 1.4 Testing; how, what, why - Dave .pdf
│   │   │   ├── 1.5 Go 业务开发中 Error & Context - 毛剑.pdf
│   │   │   ├── 1.6 Go并发编程实践 - 晁岳攀.pdf
│   │   │   ├── 1.7 百度APP Go 语言实践 - 陈肖楠.pdf
│   │   │   └── 1.8 Golang to build a real-time interactive SaaS Cloud - 董海冰.pdf
│   │   └── 2019-04-28
│   │       ├── 2.1 基于MINIO的对象存储方案在探探的实践 - 于乐.pdf
│   │       ├── 2.2 从零开始用 Go 实现 Lexer & Parser - 何源.pdf
│   │       ├── 2.3 高性能高可用的微服务框架TarsGo的腾讯实践 - 陈明杰.pdf
│   │       ├── 2.4 闪电网络—BTC小额支付解决方案 - 方圆.pdf
│   │       ├── 2.6 用Go构建高性能数据库中间件- 徐成选.pdf
│   │       ├── 2.7 花椒直播基于golang的中台技术实践 - 周洋.pdf
│   │       └── 2.8 知乎社区核心业务 Golang 化实践 - 杜旭.pdf
│   ├── 京东商城POP商品搜索原理.pdf
│   ├── 天猫推荐业务与算法架构 张奇（得福） 天猫推荐算法团队 2013·07·14 (1).pdf
│   ├── 天猫推荐业务与算法架构 张奇（得福） 天猫推荐算法团队 2013·07·14.pdf
│   ├── 推荐系统实践.pdf
│   ├── 架构专场（2）：孟凡杰-基于kubernetes的服务发现与服务治理.pdf
│   ├── 淘宝推荐系统.pdf
│   ├── 百度分布式redis集群平台开发与运维（胡东旭）.pdf
│   ├── 阿里Hadoop集群架构及服务体系.pdf
│   └── 阿里云-飞天系统-总体框架.pdf
├── 大数据
│   ├── hadoop入门手册网络版【最新版本】.pdf
│   ├── Hadoop命令大全.pdf
│   ├── hadoop官网帮助手册.pdf
│   ├── hadoop官网帮助手册【第二版】.PDF
│   ├── HADOOP实战第二版中文清晰版.pdf
│   ├── Hadoop权威指南(第二版中文).pdf
│   ├── Hadoop源代码分析(完整版).pdf
│   ├── Hive编程指南.pdf
│   ├── Netty权威指南.pdf
│   ├── 《Spark大数据商业实战三部曲：内核解密 商业案例 性能调优》.pdf
│   └── 《大数据：互联网大规模数据挖掘与分布式处理》.pdf
├── 安卓
│   ├── 12-第一行代码+Android+第2版-郭霖-人邮-2016.12-P580%40www.java1234.com(1).pdf
│   ├── 12-第一行代码+Android+第2版-郭霖-人邮-2016.12-P580%40www.java1234.com.pdf
│   ├── Android(卷2)(带全目录完整版)邓凡平(1).pdf
│   ├── Android(卷2)(带全目录完整版)邓凡平.pdf
│   ├── Android自定义组件开发详解(1).pdf
│   ├── Android自定义组件开发详解.pdf
│   ├── kotlin-for-android-developers-zh(1).pdf
│   ├── kotlin-for-android-developers-zh.pdf
│   ├── 深入理解Android 卷1.pdf
│   └── 深入理解ANDROID 卷3.pdf
├── 工具
│   ├── git-cheatsheet.pdf
│   ├── GitHub入门与实践.pdf
│   ├── Jenkins The Definitive Guide.pdf
│   ├── leaning-nginx.pdf
│   ├── OpenResty-Best-Practices.pdf
│   ├── progit-zh-v2.1.1.pdf
│   └── 深入理解Nginx模块开发与架构解析（第2版）.pdf
├── 操作系统
│   ├── APUE中文版第三版.pdf
│   ├── Linux指令速查手册.pdf
│   ├── linux编程基础.pdf
│   ├── 《Linux就该这么学》v0.2版本.pdf
│   ├── 编译原理(第3版).pdf
│   ├── 鸟哥Linux私房菜第四版.pdf
│   └── 鸟哥的Linux私房菜-基础篇.第四版.pdf
├── 数据库
│   ├── mongodb学习手册.pdf
│   ├── mongodb实战.pdf
│   ├── MongoDB权威指南.pdf
│   ├── Mycat权威指南-全部-完整版.pdf
│   ├── Mycat权威指南第一版_V1.6.0.pdf
│   ├── MySQL5.7从入门到精通@www.java1234.com.pdf
│   ├── MySQL必知必会 电子版.pdf
│   ├── MySQL必知必会.pdf
│   ├── MySQL性能调优与架构设计 PDF中文版全册.pdf
│   ├── MySQL王者晋级之路.pdf
│   ├── Redis 中文文档-20160106162932.pdf
│   ├── Redis 多机特性工作原理简介.pdf
│   ├── Redis入门手册.pdf
│   ├── redis深入研究及大集群实战经验【讲义20140510】.pdf
│   ├── The Little Redis Book中文版-20151023191222.pdf
│   ├── [MongoDB权威指南].(美).霍多罗夫.(美).迪洛尔夫.(译)程显峰.扫描版.pdf
│   ├── 《Redis实战》电子书.pdf
│   ├── 《Redis实战》红丸出品.pdf
│   ├── 大数据存储MongoDB实战指南.pdf
│   ├── 深入浅出MySQL++数据库开发、优化与管理维护+第2版+唐汉明.txt
│   ├── 深入浅出MySQL全文.pdf
│   └── 高性能mysql第三版.pdf
├── 程序最佳实践
│   ├── RESTful Web Services Cookbook.pdf
│   ├── 数据库设计开发规范-阿里.pdf
│   ├── 编写高质量代码--Web前端开发修炼之道.pdf
│   ├── 重构.pdf
│   ├── 重构_改善既有代码的设计[高清版].pdf
│   ├── 阿里巴巴Java开发手册.pdf
│   ├── 阿里巴巴Java开发手册v1.2.0.pdf
│   └── 阿里巴巴Java开发规范公开版.pdf
├── 算法数据结构
│   ├── Java数据结构和算法.（第二版）.pdf
│   ├── 大话数据结构.pdf
│   ├── 常见排序算法-20151021164549.pdf
│   ├── 挑战程序设计竞赛(第2版).pdf
│   ├── 推荐系统.pdf
│   ├── 算法导论 第三版 中文.pdf
│   ├── 算法导论.pdf
│   └── 趣学算法-608_14212712.pdf
├── 系统架构
│   ├── 一线架构师实践指南.pdf
│   ├── 企业应用架构模式.pdf
│   ├── 架构即未来：现代企业可扩展的Web架构、流程和组织.pdf
│   ├── 架构师必看——架构之美（精选版）.pdf
│   └── 架构师笔记.pdf
├── 编程语言
│   ├── go
│   │   ├── Concurrency in Go.pdf
│   │   ├── effective-go-zh-en.pdf
│   │   ├── Go API编程.pdf
│   │   ├── Go Web 编程.pdf
│   │   ├── Go Web编程.pdf
│   │   ├── go(Google)语言精要.pdf
│   │   ├── go.ogle.pdf
│   │   ├── Golang评估报告.pdf
│   │   ├── Go并发编程实战+第2版+%2C郝+林+%2CP370+%2C2017.4——正——PDF电子书下载+带书签目录+高清完整版.pdf
│   │   ├── Go并发编程实战[2015.1].pdf
│   │   ├── go教程.md
│   │   ├── Go编程基础-课堂讲义.pptx
│   │   ├── Go网络编程.pdf
│   │   ├── go语言-kindle.pdf
│   │   ├── go语言·云动力.pdf
│   │   ├── go语言圣经-zh.pdf
│   │   ├── go语言基础.pptx
│   │   ├── Go语言程序设计(programming in go)中文版.pdf
│   │   ├── GO语言编程-晨笛v1.pptx
│   │   ├── Go语言编程.pdf
│   │   ├── Go语言编程实践-part1 2.pdf
│   │   ├── Go语言编程实践-part1.pdf
│   │   ├── go高级编程.pdf
│   │   ├── knative.pdf
│   │   ├── Programming.in.Go.Creating.Applications.for.the.21st.Century.pdf
│   │   ├── [Go语言·云动力（云计算时代的新型编程语言）].樊虹剑.扫描版.pdf
│   │   ├── [Go语言程序设计].(The.Way.To.Go.A.Thorough.Introduction.To.The.Go.Programming.Language),.Balbaert,.文字版.pdf
│   │   ├── [The.Go.Programming.Language.Phrasebook(2012.4)].David.Chisnall.文字版.pdf
│   │   ├── 《Go语言实战》.pdf
│   │   ├── 七周七并发模型.pdf
│   │   ├── 学习 Go 语言(Golang).pdf
│   │   ├── 深入解析Go.pdf
│   │   └── 谢孟军 - Go API编程.pdf
│   ├── java
│   │   ├── Effective+Java+第二版+中文版.pdf
│   │   ├── java反射机制教程.pdf
│   │   ├── Java多线程编程核心技术_完整版.pdf
│   │   ├── JVM调优总结.pdf
│   │   ├── Maven权威指南_中文完整版清晰.pdf
│   │   ├── MyBatis技术内幕.pdf
│   │   ├── Spring Boot实战 ,丁雪丰 (译者) .pdf
│   │   ├── Spring Cloud微服务实战_翟永超(著) .pdf
│   │   ├── SPRING技术内幕：深入解析SPRING架构与设计原理.pdf
│   │   ├── Spring源码深度解析@www.java1234.com.pdf
│   │   ├── 《Effective Java》中文版 第2版.pdf
│   │   ├── 《java并发编程实战》(完整扫描版PDF带书签).pdf
│   │   ├── 深入实践SpringBoot.pdf
│   │   └── 深入浅出MyBatis技术原理与实战.杨开振(详细书签).pdf
│   ├── python
│   │   ├── Django Web开发指南.pdf
│   │   ├── django-new.pdf
│   │   ├── Django教程自强学堂.pdf
│   │   ├── effective-python.pdf
│   │   ├── Flask Web开发：基于Python的Web应用开发实战.pdf
│   │   ├── Flask 之旅-20160415104219.pdf
│   │   ├── fluent python.pdf
│   │   ├── Python Cookbook, 3rd Edition.pdf
│   │   ├── Python Cookbook（第三版中文）.pdf
│   │   ├── Python Web开发：测试驱动方法.pdf
│   │   ├── Python web框架.Flask中文手册.pdf
│   │   ├── Python 和 Pygame 写游戏 - 从入门到精通.pdf
│   │   ├── python3-presentation.pdf
│   │   ├── PythonTricks-译者-姚前vimiix.docx
│   │   ├── Python学习手册%28第4版%29.pdf
│   │   ├── Python数据分析与挖掘实战.pdf
│   │   ├── Python机器学习.pdf
│   │   ├── Python核心编程  （第二版）.pdf
│   │   ├── Python绝技：运用Python成为顶级黑客 扫描版带书签.pdf
│   │   ├── python绝技：运用python成为顶级黑客.pdf
│   │   ├── Python编程快速上手  让繁琐工作自动化
│   │   │   ├── automate_online-materials
│   │   │   └── Python编程快速上手  让繁琐工作自动化.pdf
│   │   ├── Python编程：从入门到实践-无损压缩.pdf
│   │   ├── Python网络数据采集.pdf
│   │   ├── The Django Book 2.0中文版-20151215230133.pdf
│   │   ├── The-Django-Book中文版.pdf
│   │   ├── 《Python Cookbook》第三版中文v2.0.0.pdf
│   │   ├── 《Python标准库》中文版.pdf
│   │   ├── 廖雪峰_Python3_ .pdf
│   │   ├── 笨办法学 Python(第四版）.pdf
│   │   ├── 简明 Python 教程 - v1.1.pdf
│   │   └── 跟老齐学python3.pdf
│   ├── scala
│   │   ├── Scala程序设计第2版.pdf
│   │   ├── Scala编程完整版.pdf
│   │   ├── spark中文手册.pdf
│   │   ├── Spark快速大数据分析.pdf
│   │   ├── 快学Scala.pdf
│   │   └── 理解Spark标准.pdf
│   └── shell
│       ├── Shell脚本学习指南 中文PDF版.pdf
│       └── Shell脚本语法.pdf
├── 网络
│   ├── HTTP.2014.05.250.pdf
│   ├── HTTP协议中文版.pdf
│   ├── HTTP协议分析.pdf
│   ├── HTTP协议详解.pdf
│   ├── HTTP权威指南[文字版][中文版].pdf
│   ├── HTTP权威指南（中文版）.pdf
│   ├── TCP_IP详解.pdf
│   ├── 图解HTTP  黑白印刷 [（日）上野宣著][人民邮电出版社][2014.05][250页].pdf
│   ├── 图解HTTP+彩色版.pdf
│   ├── 图解HTTP.pdf
│   └── 图解TCPIP第5版.pdf
├── 设计模式
│   ├── JavaScript设计模式 张容铭.pdf
│   ├── [设计模式之禅].秦小波.pdf
│   └── 设计模式_可复用面向对象软件的基础.pdf
├── 面试
│   └── 只有程序员看的懂面试圣经｜如何拿下编程面试-20151121091658.pdf
├── 项目实践
│   ├── CSS3实现“红包照片”模糊效果.pdf
│   ├── githook
│   │   ├── Git 钩子：定制工作流.pdf
│   │   ├── Hook 实战：编写动作脚本.pdf
│   │   └── 使用 Hook 将代码版本变动信息通过邮件发送.pdf
│   ├── Go语言制作Markdown预览器.pdf
│   ├── GO语言开发2048.pdf
│   ├── Hexo + github page 实现搭建博客.pdf
│   ├── HTML5实现刮刮乐效果.pdf
│   ├── Linux 多线程实现生产者消费者模式.pdf
│   ├── Linux系统安装git服务器.pdf
│   ├── Node.js+Angular实现简易聊天室.pdf
│   ├── Node使用sequelize操作mysql.pdf
│   ├── Node实现文件上传.pdf
│   ├── Node实现漂流瓶.pdf
│   ├── Node实现简单爬虫.pdf
│   ├── Node端口扫描器.pdf
│   ├── Node静态文件服务器.pdf
│   ├── OAuth 2.pdf
│   ├── py3聊天室
│   │   ├── TCP_IP协议简介与聊天程序的初步实现.pdf
│   │   └── 使用Tkinter库编写聊天程序GUI模块.pdf
│   ├── Python 实现 FTP 弱口令扫描器.pdf
│   ├── Python 实现密码强度检测器.pdf
│   ├── Python3 实现淘女郎照片爬虫.pdf
│   ├── Python实现网站模拟登陆.pdf
│   ├── py漏洞检测
│   │   ├── CDN检测插件8.pdf
│   │   ├── CMS识别(web指纹识别)扫描器开发5.pdf
│   │   ├── 基于爬虫开发E-mail收集插件2.pdf
│   │   ├── 基于爬虫开发webshell爆破插件与备份扫描插件4.pdf
│   │   ├── 基于爬虫开发XSS检测程序3.pdf
│   │   ├── 扫描器测试以及未来展望10.pdf
│   │   ├── 敏感目录爆破7.pdf
│   │   ├── 目标端口扫描，系统指纹分析6.pdf
│   │   ├── 网站爬虫开发以及SQL注入检测插件.pdf
│   │   └── 自动生成网页报告9.pdf
│   ├── Spark大数据实例开发教程.pdf
│   ├── WebSocket 协议实现.pdf
│   ├── 利用Python进行数据分析.pdf
│   ├── 利用微信API将你的微信个人号变为聊天机器人.pdf
│   ├── 基于 CSS3 实现抽奖大转盘.pdf
│   ├── 基于 Gitbook 制作电子书.pdf
│   ├── 基于 JavaScript 实现打地鼠小游戏.pdf
│   ├── 基于Koa+react开发.pdf
│   ├── 基于scrapy的天气数据采集.pdf
│   ├── 基于七牛云创建个人 hexo 博客.pdf
│   ├── 微信公众号爬虫.pdf
│   ├── 缓存组件的实现.pdf
│   ├── 网页版2048.pdf
│   ├── 网页版扫雷.pdf
│   └── 逻辑功能实现和路由限制.pdf
└── 项目管理
    └── 敏捷软件开发：原则、模式与实践.pdf

```
